...
col_name = ''
units = cm.units
vmin = 0   # or df['value'].min() across all years
vmax = 1.5 # or df['value'].max() across all years
...
if st.session_state.selected_division is not None:
    ...
    all_divisions = df_chart['division'].unique()

    # Step 2: Let user select which divisions to show
    selected_divisions = st.multiselect('Select divisions to display', options=all_divisions, default=list(all_divisions))  # or choose a subset like [all_divisions[0]]

    # Step 3: Filter data based on user selection
    filtered_df = df_chart[df_chart['division'].isin(selected_divisions)]
