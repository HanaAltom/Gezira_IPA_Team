...
sidebar_title = {
    "ar": "مؤشرات أداء الري في مشروع الجزيرة",
    "en": "Gezira Irrigation Performance Indicators",
}

crop_lst = {
    "lable_ar": "إختر نوع المحصول",
    "lable_en": "Select crop type",
    "en": ["wheats", "sorgums", "cottons"],
    "ar": ["قمح", "ذرة", "قطن"],
    "mapping_ar": {
        "قمح": "wheats",
        "ذرة": "sorgums",
...
select_season_txt = {
    "ar": "إختر موسماً",
    "en": "Select a season",
    "help_ar": "اختر الموسم الذي تود عرضه",
    "help_en": "Choose the Year/Season to visualize",
}
...
with st.sidebar:
    ...
    selected_crop = st.selectbox(crop_lst[f"lable_{language}"], crop_lst[language])
    if language != "en":
        selected_crop = crop_lst[f"mapping_{language}"][selected_crop]
    ...
    selected_season = st.selectbox(select_season_txt[language], season_list, index = 0,
                                 help=select_season_txt[f"help_{language}"])
...
piechart, titlepie = cm.plotly_pie_chart(dfca, selected_name, selected_year, language)
...
with col[0]:
    ...
    with left:
        st.markdown(f"### {indicator_map_txt[language]}")
